...
- `USE_LOCAL_FALLBACK`: Enable local LLM fallback when OpenAI is unavailable
- `LOCAL_LLM_MODEL`: Hugging Face local model for fallback
- `LOCAL_MAX_NEW_TOKENS`: Maximum tokens generated by the local model
...
```python
{
    ...
    'backend': str           # 'openai', 'local', or 'unavailable'
}
```
...
```python
{
    ...
    'backend': str,          # Active answer backend
    'llm_model': str         # Active model for selected backend
}
```
...
class MyConfig(Config):
    def __init__(self):
        ...
        self.LOCAL_LLM_MODEL = "google/flan-t5-base"
